...
export class SimpleTCPServerApp extends LoggedProcess {
  ...
  onMount(root) {
    ...
    const start = UI.button(t("app.simpletcpserver.button.start"), () => this._startFromUI(), { primary: true, icon: "fa-play" });
    /** @type {HTMLButtonElement} */
    const stop = UI.button(t("app.simpletcpserver.button.stop"), () => this._stop(), { icon: "fa-stop" });
    /** @type {HTMLButtonElement} */
