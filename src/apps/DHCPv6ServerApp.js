...
export class DHCPv6ServerApp extends LoggedProcess {
  ...
  async _handleSolicit(sockPort, req, srcIP, srcPort, duidKey, clientDUID) {
    if (!req.getOption(DHCPv6Packet.OPT_IA_NA)) return;

    const naLease = this._allocateOrReuse(duidKey);
    if (!naLease) {
      this._appendLog(`[${nowStamp()}] DHCPv6: SOLICIT duid=…${duidKey.slice(-8)} → no free address`);
      return;
    }

    this._appendLog(`[${nowStamp()}] DHCPv6: SOLICIT duid=…${duidKey.slice(-8)} → ADVERTISE ${IPAddress.fromUInt8(naLease.ip6bytes)?.toString() ?? "?"}`);

    const reply = this._buildReply(DHCPv6Packet.MT_ADVERTISE, req, clientDUID, naLease.ip6bytes);
    try {
  ...
  async _handleRequest(sockPort, req, srcIP, srcPort, duidKey, clientDUID) {
    ...
    this._appendLog(`[${nowStamp()}] DHCPv6: ${label} duid=…${duidKey.slice(-8)} → REPLY ${IPAddress.fromUInt8(naLease.ip6bytes)?.toString() ?? "?"}`);

    const reply = this._buildReply(DHCPv6Packet.MT_REPLY, req, clientDUID, naLease.ip6bytes);
    try {
      this.os.net.sendUDPSocket(sockPort, srcIP, srcPort, reply.pack());
    } catch (e) {
      const reason = e instanceof Error ? e.message : String(e);
  ...
  /**
   * Build ADVERTISE or REPLY with IA_NA + DNS options.
